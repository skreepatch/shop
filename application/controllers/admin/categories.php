...
class Categories extends CI_Controller {
	...
	$this->load->model('util/cats_processor');
	$data = $this->cats_processor->getCategories($post);

	$data['title'] = 'Admin Categories';

	$this->template->set_content('admin/categories/index', $data);
	$this->template->build();
    }
	...
	if ($id == 'save') {
	    ...
	    $success = $c->from_array($_POST, array('name', 'weight', 'favorite', 'bonus', 'active'), TRUE); // TRUE means save immediately
	    // redirect on save
	    if ($success) {
		$c->trans_complete();
		if ($id < 1) {
		    $this->session->set_flashdata('message', 'The category ' . $c->name . ' was successfully created.');
		} else {
		    $this->session->set_flashdata('message', 'The category ' . $c->name . ' was successfully updated.');
		}
	...
	if ($id > 0) {
	    $title = 'Edit Category';
	    $url = 'admin/categories/edit/save';
	} else {
	    $title = 'Add Category';
	    $url = 'admin/categories/add/save';
	}
